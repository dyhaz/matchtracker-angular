...
export class OAuthService {
   ...
   */
  createHeaderString(httpMethod: string, baseURL: string, requestParams: any, oauthKey: OAuthKey, oauthToken: OAuthToken,
  ...
  createNonce(length: number) {
    ...
    for (let i = 0; i < length; ++i) {
      const rnum = Math.floor(Math.random() * nonceChars.length);
   ...
   */
  createSignature(httpMethod: string, baseURL: string, requestParams: any, oauthKey: OAuthKey, oauthToken: OAuthToken,
                  nonce: string, timestamp: string) {
